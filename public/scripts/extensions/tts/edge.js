    ...
    */
    ...
    async throwIfModuleMissing() {
        ...
        if (this.settings.provider === EDGE_TTS_PROVIDER.plugin && !this.isPluginAvailable()) {
            const message = 'Edge TTS Server plugin not loaded. Install it from https://github.com/SuperTavern/SuperTavern-EdgeTTS-Plugin and restart the SuperTavern server.';
            // toastr.error(message)
            throw new Error(message);
        }
    }
